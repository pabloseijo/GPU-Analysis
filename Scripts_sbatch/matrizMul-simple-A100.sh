...
filas_matrices=(256 512 1024 2048)         # Tamaños de filas de A
columnas_matrices=(256 512 1024 2048)      # Tamaños de columnas de B
hilos_por_bloque=(32 64 128 256)           # Hilos por bloque (múltiplos del warp)
    ...
    for columnasB in "${columnas_matrices[@]}"; do
        for columnasA in "${columnas_matrices[@]}"; do
            for hilos in "${hilos_por_bloque[@]}"; do
                ...
                srun "$programa" "$filasA" "$columnasB" "$columnasA" "$hilos" >> "$archivo_resultados" 2>&1
